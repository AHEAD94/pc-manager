...
public class ErrorWall extends AppCompatActivity {
    ...
    EditText TextInputEditText_height;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_error_wall);
        ...
        TextView TV_height = (TextView) findViewById(R.id.tv_height_rs);
        TextView TV_width = (TextView) findViewById(R.id.tv_width_rs);
        TextView TV_thickness = findViewById(R.id.tv_thickness_rs);
        TextView TV_diagonal = findViewById(R.id.tv_diagonal_rs);
        ...
        TextInputEditText_height = findViewById(R.id.editText1);
        ...
        TextInputEditText_height.addTextChangedListener(new TextWatcher() {
            @Override
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                int hei = Integer.parseInt( "" + TextInputEditText_height.getText().toString());
                if (Math.abs(hei - height) > 5) { // 높이검사
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.button1:
                ...
                if (Math.abs(hei - height) > 5) { // 높이검사
                    heiErr = "부적합";
                    databaseReference.child("modules").child("mID"+position).child("size").child("length").setValue(TextInputEditText_height.getText().toString()+"_부적합"); // 5000
                    //lengthTv.setText("부적합");
                }
                else {
                    databaseReference.child("modules").child("mID"+position).child("size").child("length").setValue(TextInputEditText_height.getText().toString()+"_적합"); // 5000
                    heiErr = "적합";
                    //lengthTv.setText("합격");
                }
                ...
                if (thi - thickness > 5 || thi - thickness < -2) { // 두께검사
                    thiErr = "부적합";
                    databaseReference.child("modules").child("mID"+position).child("size").child("depth").setValue(TextInputEditText_thickness.getText().toString()+"_부적합"); // 304
                    //thicknessTv.setText("부적합");
                }
                else {
                    databaseReference.child("modules").child("mID"+position).child("size").child("depth").setValue(TextInputEditText_thickness.getText().toString()+"_적합"); // 304
                thiErr = "적합";
                ...
                if (dia - diagonal > 10 || dia - diagonal < -2) { // 대각선검사
                    diaErr = "부적합";
                    databaseReference.child("modules").child("mID"+position).child("size").child("depth").setValue(TextInputEditText_diagonal.getText().toString()+"_부적합"); // 304
                    //thicknessTv.setText("부적합");
                }
                else {
                    databaseReference.child("modules").child("mID"+position).child("size").child("depth").setValue(TextInputEditText_diagonal.getText().toString()+"_적합"); // 304
                    diaErr = "적합";
                    //thicknessTv.setText("합격");
                }

                Toast.makeText(getApplicationContext(), "높이 : "+heiErr+" ,  폭 : "+widErr+" ,  두께 : "+thiErr+" ,  대각선 : "+diaErr, Toast.LENGTH_LONG).show();

                if (heiErr == "적합" && widErr == "적합" && thiErr == "적합" && diaErr == "적합") {
