...
public class ConcreteClicked extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_concrete_clicked);
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.button7:
                if (this.cover == 1) {
                    databaseReference.child("requested").child("concrete").child("mID" + position).removeValue();
                    databaseReference.child("modules").child("mID" + position).child("test").child("concrete").setValue("합격");
                    ...
                    databaseReference.child("modules").child("mID" + position).child("test").child("concrete").setValue("불합격");
            ...
            case R.id.button8:
                ...
                databaseReference.child("modules").child("mID"+position).child("test").child("concrete").setValue("수선 요청됨");

                long now = System.currentTimeMillis();
                Date mDate = new Date(now);
                SimpleDateFormat simpleDate = new SimpleDateFormat("yyyy-MM-dd hh:mm");
                String getTime = simpleDate.format(mDate);
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("No").setValue("SN"+trans);
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("fabrication").child("date").child("starting").setValue("현재일시");
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("fabrication").child("date").child("finishing").setValue("생산 진행중"); // 마지막 부재검사 요청 시
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("fabrication").child("producer").setValue("현재작업자");
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("request").setValue(getTime);
                // 사진
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("photo").child("item").setValue("미등록");
                // 보고서
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("report").child("preliminary").setValue("미등록");
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("report").child("certificate").setValue("미등록");
                databaseReference.child("repairing").child("concrete").child("mID"+position).child("report").child("abrogation").setValue("미등록");
                break;
        }
    }
}
